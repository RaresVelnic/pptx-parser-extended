...
Coverage (sane, pragmatic):
- Run-level fonts: <w:rPr><w:rFonts ...> across:
    * word/document.xml
    * word/header*.xml
    * word/footer*.xml
    * word/footnotes.xml / word/endnotes.xml (if present)
- Document defaults: word/styles.xml -> w:docDefaults/w:rPrDefault/w:rPr/w:rFonts
- Style-level fonts: word/styles.xml -> w:style/w:rPr/w:rFonts
- Theme resolution: word/theme/themeX.xml mapping for w:rFonts @*Theme
    * w:asciiTheme="majorAscii" / "minorAscii"
    * w:eastAsiaTheme="majorEastAsia" / "minorEastAsia"
    * w:cstheme="majorBidi" / "minorBidi"  (map to theme's latin/ea/cs buckets)

Output schema:
[
  {"part": "document", "fonts": ["Calibri", "Arial", ...]},
  {"part": "header1",  "fonts": [...]},
  ...
]
(Parts with no fonts are omitted for cleanliness.)
"""
...
class DocxFontExtractor(DocxBaseExtractor):
    """Collect per-part font families for DOCX."""

    # -------- Theme mapping (similar spirit to PPTX) --------
    ...
    def _resolve_theme_face(self, theme_map: Dict, theme_token: Optional[str]) -> Optional[str]:
        """
        Convert @*Theme tokens into concrete font using theme_map.

        - "majorAscii" / "minorAscii"           -> bucket=major/minor, key=latin
        - "majorEastAsia" / "minorEastAsia"     -> bucket=major/minor, key=ea
        - "majorBidi" / "minorBidi"             -> bucket=major/minor, key=cs
        """
        if not theme_token:
            return None
    ...
    def _rFonts_to_names(self, z, rFonts) -> Set[str]:
        ...
        token_map = {
            ...
            "cstheme": "Bidi",   # note: schema says 'cstheme' (lowercase 'c')
        }
    ...
    def extract(self, docx_bytes: bytes) -> List[Dict]:
        ...
        with self._zip(docx_bytes) as z:
            # Start with styles-level defaults so we don't miss them
            # (we present per-part results, so styles are used as implicit fallbacks only)
            _ = self._styles_rFonts(z)  # not merged into rows, but will be resolved in runs too
